 ...
 * parameter virtualATR - VirtualATR *
 */
ClippingPlaneLocator::ClippingPlaneLocator(Vrui::LocatorTool * locatorTool,
		VirtualATR* virtualATR) :
	BaseLocator(locatorTool, virtualATR), clippingPlane(0) {
	/* Find a clipping plane index for this locator: */
	ClippingPlane * clippingPlanes = virtualATR->getClippingPlanes();
	for (int i=0; i<virtualATR->getNumberOfClippingPlanes(); ++i)
