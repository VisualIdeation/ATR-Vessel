/***********************************************************************
SimpleMCNPFluxOnly - This class reads an ASCII file which contains the results
...
These numbers are stored and then 
...
#include <Concrete/SimpleMCNPFluxOnly.h>
...
/******************************
Methods of class SimpleMCNPFluxOnly:
******************************/

/***********************************************************************
Constructor for SimpleMCNPFluxOnly class. Contains no code except the
definition of its own name.
***********************************************************************/

SimpleMCNPFluxOnly::SimpleMCNPFluxOnly(void)
	:BaseModule("SimpleMCNPFluxOnly")
	{
	}
...
***********************************************************************/
  ...
  Visualization::Abstract::DataSet* SimpleMCNPFluxOnly::load(const std::vector<std::string>& args, Comm::MulticastPipe* pipe) const
	{
	...
	/* Read all vertex positions flux values: */
	...
	for(index[0]=0;index[0]<numVertices[0];++index[0]) // I varies most slowly
		for(index[1]=0;index[1]<numVertices[1];++index[1])
			for(index[2]=0;index[2]<numVertices[2];++index[2]) // K varies fastest
				{
				/* Read the vertex position, flux value: */
                                  double pos[3],flux;
...
extern "C" Visualization::Abstract::Module* createFactory(Plugins::FactoryManager<Visualization::Abstract::Module>& manager)
	...
	Visualization::Concrete::SimpleMCNPFluxOnly* module=new Visualization::Concrete::SimpleMCNPFluxOnly();
